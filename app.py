# app.py
# The Adbook — AIAMS (Supabase + Streamlit Cloud SAFE)
# ✅ No xhtml2pdf / pycairo dependencies
# ✅ Uses ReportLab for PDF generation (works on Streamlit Cloud)
# ✅ Uses psycopg (v3) driver via SQLAlchemy: postgresql+psycopg://
# ✅ Permissions table uses safe column names (role_name / section_name)
# ✅ First-time login supported with plain$ passwords
...
def _normalize_db_url(db_url: str) -> str:
    """
    Ensures SQLAlchemy uses psycopg (v3) driver.

    Accepts:
      - postgresql://...
      - postgres://... (older style)
...
def migrate():
    ...
    # IMPORTANT: use safe column names role_name/section_name (avoids edge keyword conflicts)
    exec_sql(
        """
...
def load_permissions():
    # Map safe columns back to role/section names used in code
    return qdf(
        """
        SELECT
          role_name AS role,
...
def require_auth():
    ...
    with st.sidebar:
        ...
        st.caption("If first time: create SuperAdmin user in Supabase SQL Editor (see bottom of sidebar).")
        st.stop()
...
def upsert_lead_update(section, record_hash, status, assigned_to, lead_source, notes, follow_up, last_call_outcome=None):
    exec_sql(
        """
        INSERT INTO lead_updates_v8(
          record_hash,section,status,assigned_to,seen_by,
          lead_source,notes,follow_up,last_call_outcome,last_call_at,last_updated
        )
        VALUES(:h,:s,:st,:as,NULL,:src,:n,:fu,:out,CASE WHEN :out IS NULL THEN NULL ELSE NOW() END,NOW())
...
# NOTE:
# If your existing DB already has lead_updates_v8 without the "seen_by" column,
# you can remove "seen_by" from the insert above OR add it via Supabase SQL:
# ALTER TABLE lead_updates_v8 ADD COLUMN IF NOT EXISTS seen_by TEXT;
...
def get_company_settings():
    ...
    if len(df) == 0:
        sid = str(uuid.uuid4())
        exec_sql(
            "INSERT INTO company_settings_v8(settings_id, gst_no, bank_details, whatsapp_limit_per_hour) "
            "VALUES(:i,'','',50)",
            {"i": sid},
        )
        df = qdf("SELECT * FROM company_settings_v8 LIMIT 1")
...
def get_user_profile(username: str):
    ...
    if len(df) == 0:
        exec_sql(
            "INSERT INTO user_profiles_v8(username, signature_filename, designation, mobile, email) "
            "VALUES(:u,'','','','')",
            {"u": username},
        )
        df = qdf("SELECT * FROM user_profiles_v8 WHERE username=:u", {"u": username})
...
def update_user_signature(username: str, filename: str):
    exec_sql(
        "UPDATE user_profiles_v8 SET signature_filename=:f, updated_at=NOW() WHERE username=:u",
        {"u": username, "f": filename},
    )


# =========================
# PDF PROPOSALS (REPORTLAB)
# =========================
def make_proposal_pdf_bytes(section: str, data: dict, settings: dict, signer: dict, validity_days: int = 15) -> bytes:
    ...
    line(
        f"Contact: {data.get('contact_person','')} | {data.get('contact_phone','')} | {data.get('contact_email','')}",
        11,
        False,
        16,
    )
...
def whatsapp_rate_ok(username: str, limit_per_hour: int) -> bool:
    df = qdf(
        "SELECT COUNT(*) AS c FROM whatsapp_logs_v8 WHERE username=:u AND created_at >= (NOW() - INTERVAL '1 hour')",
        {"u": username},
    )
...
if PAGE == "Home":
    ...
    if can(SECTION_INSTALL, "edit", ROLE):
        ...
        if st.button("Mark serviced", type="primary"):
            exec_sql(
                "UPDATE screens_v8 SET last_service_date=:ls, next_service_due=:nd, last_updated=NOW() WHERE screen_id=:sid",
    ...
    if SECTION == SECTION_INSTALL:
        ...
        st.info("Ads documents: add advertisers/billing flows next (kept minimal in this build).")

elif PAGE == "Proposals":
    page_title("📄 Proposals", "Generate proposal PDF (Installation).")
    ...
    if SECTION == SECTION_INSTALL:
        ...
        if st.button("Generate Installation Proposal PDF", type="primary", disabled=not can(SECTION_INSTALL, "add", ROLE)):
            data = {
                ...
                "pricing_rows": [
                    {"item": "Agreed Rent (per month)", "amount": f"{rent:,.0f}" if rent else "", "notes": invr.get("contract_period", "")}
                ],
        ...
        st.info("Ads proposals are part of the full build; this build keeps Ads minimal.")

    st.markdown("---")
    hist = qdf(
        "SELECT section, proposal_no, property_id, advertiser_id, pdf_filename, created_by, created_at, status "
        "FROM proposals_v8 ORDER BY created_at DESC LIMIT 200"
    )
    ...
    st.link_button("Open WhatsApp", whatsapp_url(phone, msg), use_container_width=True, disabled=(not ok_rate) or (not bool(normalize_mobile(phone))))

    if st.button("Mark Sent ✅", type="primary"):
        exec_sql(
            "INSERT INTO whatsapp_logs_v8(log_id,campaign_id,lead_hash,username,action_status) VALUES(:id,'manual',:h,:u,'Sent')",
...
st.sidebar.code(
    "INSERT INTO users(username, password_hash, role, section_scope, is_active)\n"
    "VALUES ('admin', 'plain$admin123', 'SuperAdmin', 'Both', 1)\n"
