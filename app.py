...
if PAGE_KEY == "Home":
    ...
    inv = qdf("SELECT * FROM inventory_sites ORDER BY last_updated DESC LIMIT 2000")
    if q.strip():
        s = q.strip().lower()
        inv["__s"] = (
            inv.fillna("").astype(str)
            .agg(" | ".join, axis=1)
            .str.lower()
        )
        inv = inv[inv["__s"].str.contains(re.escape(s), na=False)].drop(columns=["__s"])
    ...
    if q.strip():
        s = q.strip().lower()
        scr["__s"] = scr.fillna("").astype(str).agg(" | ".join, axis=1).str.lower()
        scr = scr[scr["__s"].str.contains(re.escape(s), na=False)].drop(columns=["__s"])
    ...
    ad = qdf("SELECT * FROM ad_inventory WHERE section=:s ORDER BY updated_at DESC LIMIT 3000", {"s": SECTION})
    if q.strip():
        s = q.strip().lower()
        ad["__s"] = ad.fillna("").astype(str).agg(" | ".join, axis=1).str.lower()
        ad = ad[ad["__s"].str.contains(re.escape(s), na=False)].drop(columns=["__s"])
    ...
    ag = qdf("SELECT * FROM agreements WHERE section=:s ORDER BY updated_at DESC LIMIT 3000", {"s": SECTION})
    if q.strip():
        s = q.strip().lower()
        ag["__s"] = ag.fillna("").astype(str).agg(" | ".join, axis=1).str.lower()
        ag = ag[ag["__s"].str.contains(re.escape(s), na=False)].drop(columns=["__s"])
    ...
    page_title("💰 Billing & Reminders", "Track due payments, paid status and reminders.")
    ...
    dues = qdf(
        """SELECT p.*, a.party_name, a.property_code
           ...
           WHERE p.section=:s AND COALESCE(NULLIF(p.due_date,''),'9999-12-31') <= :cutoff
           ORDER BY p.due_date ASC
           LIMIT 5000""",
        {"s": SECTION, "cutoff": cutoff},
    )
    k1,k2,k3 = st.columns(3)
    with k1:
        kpi("Due rows", f"{len(dues):,}")
    with k2:
        kpi("Overdue (unpaid)", f"{int(((dues.get('status')=='Unpaid') & (dues.get('due_date','')<today)).sum()) if len(dues) else 0:,}")
    with k3:
        kpi("Paid", f"{int((dues.get('status')=='Paid').sum()) if len(dues) else 0:,}")

    st.dataframe(dues, use_container_width=True, height=420)

    st.markdown("### ➕ Add payment / due")
    if not can(SECTION, "add", ROLE):
        st.info("No add permission.")
    else:
        ag = qdf("SELECT agreement_id, property_code, party_name FROM agreements WHERE section=:s ORDER BY updated_at DESC LIMIT 5000", {"s": SECTION})
        ag_opts = ag["agreement_id"].fillna("").astype(str).tolist()
        with st.form("pay_form"):
            c1,c2,c3 = st.columns(3)
            with c1:
                payment_id = str(uuid.uuid4())
                agreement_id = st.selectbox("agreement_id", [""] + ag_opts, index=0)
                property_id = st.text_input("property_id (optional)")
            with c2:
                due_date = st.text_input("due_date (YYYY-MM-DD)", value=(date.today()+timedelta(days=7)).isoformat())
                amount = st.number_input("amount", min_value=0.0, value=0.0, step=100.0)
                status = st.selectbox("status", ["Unpaid","Paid","Partial","Cancelled"], index=0)
            with c3:
                paid_date = st.text_input("paid_date (YYYY-MM-DD)", value="")
                payment_mode = st.text_input("payment_mode", value="")
                reference_no = st.text_input("reference_no", value="")
            notes = st.text_area("notes", value="", height=80)
            ok = st.form_submit_button("Add", type="primary")
        if ok:
            exec_sql(
                """INSERT INTO payments(payment_id,agreement_id,section,property_id,due_date,amount,status,paid_date,payment_mode,reference_no,notes,created_by)
                   VALUES(:pid,:aid,:sec,:prop,:due,:amt,:st,:paid,:mode,:ref,:notes,:by)
                """,
                {
                    "pid": payment_id,
                    "aid": agreement_id or None,
                    "sec": SECTION,
                    "prop": property_id or None,
                    "due": due_date,
                    "amt": float(amount),
                    "st": status,
                    "paid": paid_date,
            ...
            audit(USER, "ADD_PAYMENT", f"{SECTION} {payment_id}")
            st.success("Added.")
            st.rerun()
    ...
    docs = qdf("SELECT * FROM documents_vault WHERE section=:s ORDER BY uploaded_at DESC LIMIT 3000", {"s": SECTION})
    if q.strip():
        s = q.strip().lower()
        docs["__s"] = docs.fillna("").astype(str).agg(" | ".join, axis=1).str.lower()
        docs = docs[docs["__s"].str.contains(re.escape(s), na=False)].drop(columns=["__s"])
