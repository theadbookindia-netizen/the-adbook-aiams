...
def get_database_url() -> str:
    """
    Reads DATABASE_URL safely from Streamlit Secrets or env var.
...
# Seed permissions
# =========================
def seed_permissions_once() -> None:
    try:
...
def audit(user: str, action: str, details: str = "") -> None:
    exec_sql(
...
def normalize_mobile(x: Any) -> str:
...
def make_hash(prop: str, addr: str, dist: str, city: str, mobile: str, email: str) -> str:
    s = "||".join([prop, addr, dist, city, mobile, email]).lower().encode("utf-8", errors="ignore")
    return hashlib.sha256(s).hexdigest()


def whatsapp_url(mobile: str, message: str) -> str:
...
def ensure_property_codes(leads_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for r in needed.to_dict("records"):
        ...
        prefix = _letters2(str(r.get("district", ""))) + _letters2(str(r.get("city", "")))
        ...
        rows_to_insert.append(
            {
                ...
                "city": str(r.get("city", "")),
                "property_name": str(r.get("property_name", "")),
            }
        )
...
def property_display_map(code_df: pd.DataFrame, leads_df: pd.DataFrame) -> Dict[str, str]:
...
def upsert_company_settings(gst_no: str, bank_details: str, limit_per_hour: int) -> None:
...
def update_user_signature(username: str, filename: str) -> None:
    exec_sql("UPDATE user_profiles_v8 SET signature_filename=:f, updated_at=NOW() WHERE username=:u", {"u": username, "f": filename})


# =========================
# PDF generator (ReportLab)
# =========================
def make_proposal_pdf_bytes(
    section: str,
    data: Dict[str, Any],
    ...
    def line(txt: str = "", size: int = 11, bold: bool = False, gap: int = 16) -> None:
        nonlocal y
        if y < 60:
            c.showPage()
            y = height - 40
...
def next_proposal_no() -> int:
    df = qdf("SELECT MAX(proposal_no) AS m FROM proposals_v8")
    m = df.iloc[0]["m"]
    return int(m or 0) + 1


def save_proposal_pdf(section: str, property_id: str, advertiser_id: Optional[str], pdf_bytes: bytes, created_by: str) -> Dict[str, Any]:
...
with st.sidebar:
    ...
    MENU_ADS = ["Home", "Leads", "Proposals", "WhatsApp", "Reports"]  # minimal
...
if PAGE == "Home":
    ...
    if SECTION != SECTION_INSTALL:
        st.info("Ads documents are minimal in this build. Switch to Installation module.")
        st.stop()
    ...
    signer = get_user_profile(USER)
    signer["username"] = USER

    if SECTION != SECTION_INSTALL:
        st.info("Ads proposal is minimal in this build. Switch to Installation module.")
        st.stop()
    ...
    if st.button(
        "Generate Installation Proposal PDF",
        type="primary",
        disabled=not can(SECTION_INSTALL, "add", ROLE),
    ):
        data = {
            ...
            "pricing_rows": [
                {
                    "item": "Agreed Rent (per month)",
                    "amount": f"{rent:,.0f}" if rent else "",
                    "notes": invr.get("contract_period", ""),
                }
            ],
    ...
    st.markdown("---")
    hist = qdf(
        "SELECT section, proposal_no, property_id, advertiser_id, pdf_filename, created_by, created_at, status FROM proposals_v8 ORDER BY created_at DESC LIMIT 200"
    )
...
st.sidebar.code(
    ...
    "-- Replace SALT and HASH. Ask me and I will generate for your password.",
    language="sql",
)
